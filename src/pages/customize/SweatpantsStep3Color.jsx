// src/pages/customize/SweatpantsStep3Color.jsx
...
import sweatpantsDere from "../../assets/hoodieParts/franja-dere.webp";
import sweatpantsizq from "../../assets/hoodieParts/franja-izq.webp";

// Mapeo maestro de datos de piezas por silueta
const SweatpantsModelsData = {
  sweatpants1: {
    label: "CLASSIC",
    parts: [{ name: "Frente y espalda basica", image: sweatpants1 }],
  },
  sweatpants2: {
    label: "SWEATPANTS OVERSIZE",
    parts: [{ name: "Frente y espalda Oversize", image: sweatpants2 }],
  },
  sweatpants3: {
    label: "FRANJAS LATERALES",
    parts: [
      { name: "Frente y espalda Franjas", image: sweatpants3 },
      { name: "Franja derecha", image: sweatpantsDere },
      { name: "Franja izquierda", image: sweatpantsizq },
    ],
  },
};
...
export default function SweatpantsStep3Color() {
  const navigate = useNavigate();
  const { state } = useLocation();

  // Recupera el tamaño y modelo del paso anterior de manera consistente
  ...
  const currentModel =
    SweatpantsModelsData[selectedModelKey] ||
    SweatpantsModelsData["sweatpants2"];

  const sweatpantsParts = currentModel.parts;

  // Estados locales
  const [selectedPart, setSelectedPart] = useState(null);
  const [colors, setColors] = useState({});
  const [showPreview, setShowPreview] = useState(false);
  // Inicializamos en 0 para que el primer acordeón (Cuerpo Principal) abra automáticamente
  ...
  // Reinicio limpio de estados al cambiar el modelo en el router
  useEffect(() => {
    setColors({});
    ...
    if (sweatpantsParts && sweatpantsParts.length > 0) {
      setSelectedPart(sweatpantsParts[0].name);
    } else {
  ...
  const handleNextClick = () => {
    ...
    // Avanza al paso 4 (o al carrito/resumen) transmitiendo todo el estado acumulado
    navigate("/customize/sudaderas/step4", {
      state: {
        selectedSweatpants: selectedModelKey,
        selectedSize,
        ...
        <div className="nap-hero relative z-10">
          <div className="nap-hero-eyebrow flex items-center justify-center md:justify-start gap-3">
            ...
            <span className="nap-hero-tag whitespace-nowrap">
              — PASO 3 ({currentModel.label.toUpperCase()})
            </span>
          </div>
        ...
        <div className="nap-body max-w-7xl mx-auto px-4 w-full mt-4 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start w-full">
            ...
            <div className="md:col-span-5 flex flex-col items-center justify-center md:sticky md:top-6">
              ...
              >
                <div className="relative w-full h-[380px] sm:h-[450px]">
                  {sweatpantsParts.map(({ name, image }) => (
            ...
            <div className="md:col-span-7 w-full flex flex-col gap-4">
              ...
              >
                {[
                  { title: "CUERPO PRINCIPAL", filters: ["frente y espalda"] },
                  {
                    title: "FRANJAS LATERALES",
                    filters: ["franja"],
                    exclude: ["frente"],
                  }, // <-- Agregamos una regla de exclusión
                ].map(({ title, filters, exclude }, i) => {
                  const isOpen = openSection === i;

                  // Modificamos el filtro para respetar la exclusión si existe
                  const filteredParts = sweatpantsParts.filter(({ name }) => {
                    const nameLower = name.toLowerCase();
                    const matchesFilter = filters.some((f) =>
                      nameLower.includes(f.toLowerCase()),
                    );
                    const matchesExclude = exclude
                      ? exclude.some((e) => nameLower.includes(e.toLowerCase()))
                      : false;

                    return matchesFilter && !matchesExclude;
                  });
